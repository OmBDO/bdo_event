...
class AnalyticsDashboard extends StatelessWidget {
  const AnalyticsDashboard({
    ...
    required this.isLoadingCheckIns,
    super.key,
  });
  ...
  Widget build(BuildContext context) {
    ...
    final conversion = registered == 0
        ? 0.0
        : (checkedIn / registered).clamp(0.0, 1.0);
          ...
          children: [
            ...
            AnalyticsMetricGrid(
              ...
              metrics: [
                ...
                AnalyticsMetricData(
                  'Checked in',
                  isLoadingCheckIns ? '--' : '$checkedIn',
                  Icons.how_to_reg_rounded,
                  palette.coral,
                  '${(conversion * 100).round()}% conversion',
                ),
                AnalyticsMetricData(
                  'Capacity',
                  capacity?.toString() ?? 'Open',
                  Icons.event_seat_outlined,
                  palette.gold,
                  capacity == null
                      ? 'no limit'
                      : '${(fill! * 100).round()}% filled',
                ),
                AnalyticsMetricData(
                  'Remaining',
                  remaining?.toString() ?? '--',
            ...
            if (isWide)
              Row(
                ...
                children: [
                  Expanded(
                    flex: 3,
                    child: AnalyticsChartPanel(event: event, palette: palette),
                  ),
    ...
    children: [
      Expanded(
        child: Column(
          ...
          children: [
            Text(
              AppText.operationsEventAnalysis,
              style: TextStyle(
                color: palette.teal,
            ...
            Text(
              ...
              style: TextStyle(color: palette.muted, fontSize: AppSize.text13),
            ),
          ],
        ),
      ),
