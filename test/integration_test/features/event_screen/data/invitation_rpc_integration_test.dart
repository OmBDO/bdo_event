...
void main() {
  ...
  Future<EventFixtureData> createEvent({
    ...
    final event = EventFixtureData(
      event: draft.event.copyWith(endTime: endTime, isAvailable: isAvailable),
      testId: draft.testId,
    );
  ...
  Future<void> sendInvitation(EventFixtureData event) => adminClient.rpc(
  ...
  Future<void> respondToInvitation(EventFixtureData event) => inviteeClient.rpc(
  ...
  Future<String> invitationStatus(EventFixtureData event) async {
  ...
  Future<List<dynamic>> activeRegistrations(EventFixtureData event) =>
      cleanupClient
  ...
  test(
    ...
    () async {
      final capacityEvent = await createEvent(
        testId: 'invitation-capacity-event',
        capacity: 1,
      );
      await activate(regularClient, capacityEvent);
      await sendInvitation(capacityEvent);

      await expectLater(
        respondToInvitation(capacityEvent),
        throwsA(
      ...
      expect(await invitationStatus(capacityEvent), 'pending');
      expect(await activeRegistrations(capacityEvent), hasLength(1));

      final deadlineEvent = await createEvent(
        testId: 'invitation-deadline-event',
        registrationDeadline: DateTime.now().toUtc().subtract(
          const Duration(minutes: 1),
        ),
      );
      await sendInvitation(deadlineEvent);

      await expectLater(
        respondToInvitation(deadlineEvent),
        throwsA(
          predicate<supabase.PostgrestException>(
      ...
      expect(await invitationStatus(deadlineEvent), 'pending');
      expect(await activeRegistrations(deadlineEvent), isEmpty);

      final finishedEvent = await createEvent(
        testId: 'invitation-finished-event',
        date: '01/01/2020',
      );
      await sendInvitation(finishedEvent);

      await expectLater(
        respondToInvitation(finishedEvent),
        throwsA(
          predicate<supabase.PostgrestException>(
      ...
      expect(await invitationStatus(finishedEvent), 'pending');
      expect(await activeRegistrations(finishedEvent), isEmpty);

      final now = DateTime.now().toUtc();
      final finishedByTimeEvent = await createEvent(
        testId: 'invitation-finished-by-time-event',
        date: '${now.day}/${now.month}/${now.year}',
        endTime: '00:00',
      );
      await sendInvitation(finishedByTimeEvent);

      await expectLater(
        respondToInvitation(finishedByTimeEvent),
        throwsA(
          predicate<supabase.PostgrestException>(
            (error) => error.message.contains('ended'),
          ),
      ...
      expect(await invitationStatus(finishedByTimeEvent), 'pending');
      expect(await activeRegistrations(finishedByTimeEvent), isEmpty);

      final unavailableEvent = await createEvent(
        testId: 'invitation-unavailable-event',
        isAvailable: false,
      );
      await sendInvitation(unavailableEvent);

      await expectLater(
        respondToInvitation(unavailableEvent),
        throwsA(
          predicate<supabase.PostgrestException>(
      ...
      expect(await invitationStatus(unavailableEvent), 'pending');
      expect(await activeRegistrations(unavailableEvent), isEmpty);
    },
  );

  test('revoked admin roles cannot use an already-issued session', () async {
    final event = await createEvent(testId: 'stale-admin-event');
    await SupabaseActorRoleUpdater(environment: environment)
        .setRoles(admin.userId!, const ['user']);
